...
def output_word_topic_dist(lda, voca):
    ...
    with open("data/word_topic_dist5k.txt","w") as outFile:
        for k in xrange(lda.K):
            ...
            for w in numpy.argsort(-phi[k])[:20]:
                if tag_dict[voca[w]] != 0:
                    ...
                    print >>outFile, "%s: %f (%d)" % (voca[w], phi[k,w], wordcount[k].get(w,0))


    tags = []
    topic_dist = lda.n_m_z / lda.n_m_z.sum(axis=1)[:,numpy.newaxis]
    
    print "pickling shit!!!!!!!!!!!!!!!!"
    output=open('superdata0.05-33k-150-20.pkl','wb')
    pickle.dump(topic_dist,output)
    pickle.dump(phi,output)
    pickle.dump(voca,output)
    output.close()
    print "pickling done!!!!!!!!!!!!!!!!"
    """
    for i in xrange(topic_dist.shape[0]):
        tags.append([])
        ##print "Doc " + str(i) + ": " 
        #top5 = topic_dist[i].argsort()[-10:0]
        top5 = numpy.argsort(topic_dist[i])
        top5 = top5[len(top5)-5:len(top5)]   ##5
        for topic in top5:
            wc = 0
            for w in numpy.argsort(-phi[topic]):    ##[:5]:  
                if tag_dict[voca[w]] != 0:
                    if(wc>6):                   ##wc>4
                        break
                    ##print "%s: %f (%d)" % (voca[w], phi[topic,w], wordcount[topic].get(w,0))
                    tags[i].append(voca[w])
                    wc += 1
    
    with open("PT15k-150-15-56.txt","w") as tag_output:
        for i in xrange(topic_dist.shape[0]):
